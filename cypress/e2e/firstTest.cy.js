describe('first suite', function () {
    ...
    it('check boxes', function () {
        ...
        cy.get('[type="checkbox"]').check({force: true}) // cy will check the checkbox only if it was uncheked previosly, also it will go through all 3 elements
        cy.get('[type="checkbox"]').eq(0).click({force: true})

    });

    it('datepickers', function () {
    ...
    it('lists and dropdowns', function () {

        cy.visit('/')
        // 1
        // cy.get('button.select-button').click()
        // cy.get('ul.options-list').contains('Dark').click()
        // cy.get('button.select-button').should('contain', 'Dark')
